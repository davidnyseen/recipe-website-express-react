...
module.exports.test_post =  async (req, res) => {
  const user = await User.login('dyn1565@gmail.com', '12345678');
  const token = createToken(user._id);
  res.cookie('jwt', token,
   { httpOnly: true, maxAge: maxAge * 1000, path: '/', domain: 'http://localhost:3000/',});
  res.send('hell yea').status(200);
...
module.exports.login_post = async (req, res) => {
  ...
  try {
    ...
    res.cookie('jwt', token,
     { httpOnly: true, maxAge: maxAge * 1000, path:"/", secure: true, proxy : true,});
    res.status(200).send({ user: user.name, id: user._id, recommendations: user.recommendationSet });
  } 
  catch (err) {
    const errors = handleErrors(err);
    res.status(400).json({ errors });
